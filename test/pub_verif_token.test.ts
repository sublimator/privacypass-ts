...
test.each(vectors)('PublicVerifiable-Vector-%#', async (v: Vectors) => {
    ...
    const [state, tokReq] = await client.createRequest(tokChl, publicKeyEnc);
    testSerialize(TokenRequest, tokReq);

    const tokReqSer = tokReq.serialize();
    expect(uint8ToHex(tokReqSer)).toBe(v.token_request);

    const issuer = new Issuer('issuer.example.com', privateKey, publicKey, mode);
    ...
    expect(await verifyToken(token, issuer.publicKey, mode)).toBe(true);
